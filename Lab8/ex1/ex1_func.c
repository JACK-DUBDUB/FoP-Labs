...
void handle_readSalaries(SalaryArray salaries)
{
    ...
    do 
    {
        ...
        if(read_userFloat(&salaries.data[index]) == 0)
        {
            index++;
        } 
...
void handle_getSalaryMinMax(const SalaryArray salaries) 
{
    int min = 0, max = 0;
...
void get_salaryMinMax(const SalaryArray salaries, int *min, int *max)
{
...
void handle_getSalaryAverage(const SalaryArray salaries)
{
    float salary_sum = 0;
...
float get_salaryAverage(const SalaryArray salaries, float *salary_sum)
{
    for (int i = 0; i < salaries.count; i++)
    {
...
void handle_getEmployeeSalaries(const SalaryArray salaries)
{
    int quit_program = 1;
    do {
        search_employeeSalaries(salaries);
        quit_program =handle_exit();
    } while (!quit_program);
    return;
}
...
void search_employeeSalaries(SalaryArray salaries)
{
    ...
    if(read_userFloat(&search_value) == 0) 
    {
